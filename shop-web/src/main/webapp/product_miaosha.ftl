...
function defaultProductImg(){
	...
	<div id="wrap">
					...
					<div class="row" style="margin-top: 10px;">
						...
						<div class="col-xs-6">
							<div style="line-height: 20px;">
								<div style="color:#F00;font-size:18px">秒杀开始时间： ${e.miaoshaStartTime!""}
							        <div id="miaoshaTip">	
							        	<input type="hidden" id="remainSeconds" value="${remainSeconds}" />
							        	<#if miaoshaStatus == 0>秒杀倒计时：<span id="countDown" text="${remainSeconds}"></span>秒</#if>
							        	<#if miaoshaStatus == 1>秒杀进行中</#if>
							        	<#if miaoshaStatus == 2>秒杀已结束</#if>
							        </div>
						        </div>
								...
								<#if e.specJsonString??>
									...
									<div class="row" style="margin-bottom: 10px;">
										<div class="col-xs-12">
											...
											<input value="1" size="4" maxlength="4" name="inputBuyNum" id="inputBuyNum" style="text-align: center;" disabled="disabled"/>
		...
		<!-- 										<button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button> -->
									...
									<div class="row">
										<div class="col-xs-12">
											<button class="btn btn-primary btn-sm" data-toggle="show" data-placement="top" value="立即秒杀" id="confirmOrderBtn" type="button" onclick="getMiaoshaToken()">立即秒杀</button>
		        							<input id="verifyCode"  size="6" maxlength="6"  style="display:none" />
											<img id="verifyCodeImg" width="80" height="30"  style="display:none" onclick="refreshVerifyCode()"/>
										</div>
									</div>
								<br>
								${qqHelpHtml!""}
...
<script>
$(function() {
	$("#miaoshaBtn").removeAttr("disabled");
...
function countDown(){
	...
	if(remainSeconds > 0){//秒杀还没开始，倒计时
		...
		$("#verifyCodeImg").attr("src", "${basepath}/miaosha/verifyCode?productId="+$("#productId").val());
		<#if currentAccount()??>
            $("#verifyCodeImg").show();
			$("#verifyCode").show();
        </#if>
...
function refreshVerifyCode(){
	$("#verifyCodeImg").attr("src", "${basepath}/miaosha/verifyCode?productId="+$("#productId").val()+"&timestamp="+new Date().getTime());
}
			...
			if(data.code == 0){
				...
				if(result < 0){
					...
					layer.confirm("恭喜你，秒杀成功！查看订单？", {btn:["确定","取消"]},
							function(){
								window.location.href="${basepath}/paygate/pay?orderId="+result.id + "&orderPayId="+result.orderpayID;
							},
							function(){
								layer.closeAll();
							});
